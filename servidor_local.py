...
def now_ar(): return datetime.now(TZ_AR)

REFRESH_SECONDS    = 600   # PF: cada 10 minutos
...
def formatear_reporte(bancos: list, cambios: list = None,
    ...
    tipo     = "ALERTA DE CAMBIO" if is_alert else "PLAZO FIJO WEB"
    ...
    if mer:
        lider_txt = "líder del mercado" if pos_mer == 1 else f"gap {mer['tna30'] - bancos_no_nacion[0]['tna30']:+.2f}%"
        lines += [
    ...
    if is_alert and (cambios or tasa_nacion_previa is not None):
        ...
        if tasa_nacion_previa is not None and nacion:
            nc = nacion["tna30"] - tasa_nacion_previa
            arrow = "▲" if nc > 0 else "▼"
            mov.append(f"`{arrow} Nación        {tasa_nacion_previa:.2f} → {nacion['tna30']:.2f}  ({nc:+.2f}%)`")
        for banco_n, tna_ant, tna_new in (cambios or []):
            diff  = tna_new - tna_ant
            arrow = "▲" if diff > 0 else "▼"
            mov.append(f"`{arrow} {banco_n:<13} {tna_ant:.2f} → {tna_new:.2f}  ({diff:+.2f}%)`")
        lines += mov
        lines += ["", DIV, ""]
    ...
    for b in top5:
        ...
        pos_str = f"#{pos}" if pos != prev_pos else "  "
        prev_pos = pos
        lines.append(f"`{pos_str}  {b['nombre']:<16} {b['tna30']:>6.2f}%{marker}`")
...
def enviar_alerta_admin(mensaje: str):
    """Envía alerta de error al admin via Telegram."""
    ...
    try:
        ...
        body = urllib.parse.urlencode({
            ...
            "text":       f"⚠️ *ALERTA SERVIDOR*\n{mensaje}",
            "parse_mode": "Markdown",
        }).encode()
        req = urllib.request.Request(url, data=body)
        urllib.request.urlopen(req, timeout=10)
        log.info(f"✅ Alerta admin enviada")
    except Exception as e:
        log.error(f"❌ Error enviando alerta admin: {e}")
...
async def refresh_pf():
    ...
    while True:
        ...
        try:
            ...
            enviar_alerta_admin(f"❌ PF scraping falló\n`{e}`")
    ...
    while True:
        ...
        try:
            ...
            if data:
                ...
                cache["dolar"]["ts"]    = now_ar().isoformat()
            ...
            enviar_alerta_admin(f"❌ Dólar scraping falló\n`{e}`")
...
async def iamc_procesar(fecha: "date") -> bool:
    ...
    if result.returncode != 0:
        ...
        enviar_alerta_admin(f"❌ IAMC error generando PDF\n`{result.stderr[:200]}`")
        return False
    ...
    while True:
        ...
        if not iamc_ya_procesado(fecha):
            ...
            if ok:
                ...
                manana = now_ar().replace(hour=IAMC_HORA_INICIO, minute=IAMC_MIN_INICIO, second=0) + timedelta(days=1)
                espera = (manana - now_ar()).total_seconds()
