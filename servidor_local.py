...
REFRESH_SECONDS  = 60  # ambos endpoints refrescan cada 1 minuto
...
async def get_pf():
    if cache["pf"]["data"]:
        return {"ok": True, "source": "local-bcra", "data": cache["pf"]["data"], "ts": cache["pf"]["ts"]}
    return {"ok": False, "error": cache["pf"]["error"] or "Esperando primer scraping..."}

@app.get("/dolar")
async def get_dolar():
    if cache["dolar"]["data"]:
        return {"ok": True, "source": "local-scraping", "data": cache["dolar"]["data"], "ts": cache["dolar"]["ts"]}
    return {"ok": False, "error": cache["dolar"]["error"] or "Esperando primer scraping..."}
